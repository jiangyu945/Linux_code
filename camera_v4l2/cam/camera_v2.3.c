/*
 ...
 ============================================================================
 ==========================ChangeLog=========================================
    1.修改select监听为epoll监听---2018-12-17
    2.添加网络功能，加入发送图片（至QT端显示）---2018-12-25
 */
...
void tcp_init()
{
	...
	bind( sfd, (struct sockaddr *)&saddr, 16 );
	listen( sfd, 100 );
...
/*获取一帧图像*/
static int read_frame(int epfd)
{
    struct v4l2_buffer buf;
    ...
    ioctl(cam_fd, VIDIOC_DQBUF, &buf);  //出队，从队列中取出一个缓冲帧
        ...
        uint datasize = buffers[buf.index].length;
        send(connfd,&datasize,sizeof(datasize),0);   //发送图片大小
        int count = send(connfd,buffers[buf.index].start,datasize,0);   //发送数据
        printf("send count = %d\n",count);
        if(count == 0){
            //连接断开，关闭套接字
            close(connfd);
            ...
            if(nfds > 0)
            {
                for(i=0;i<nfds;i++){
                    ...
                    if( (events[i].data.fd == sfd) && (events[i].events & EPOLLIN) ){
                        ...
                        if( (connfd=accept(sfd,(struct sockaddr*)&caddr,&addr_len)) > 0 )
                        {
                    ...
                    //2.监听摄像头数据的采集
                    else if(events[i].data.fd == cam_fd){
