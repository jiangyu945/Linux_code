/*
 ...
 Version     : v1.0
 date        : 2018-10-10
 Description : Multipthread Socket Server (add req )
 ============================================================================
 */
#include <stdio.h>
#include <stdlib.h>
...
int get_one_frame(int sockfd)
{
    ...
    unsigned char recv_tmp[1024] = {0};
	int nByte,get_complete_flag=0,get_head_flag=0,count;
	int i,len;

    while(1)
    {
        //接收数据  
        nByte = recv( sockfd, recv_tmp, REQ_FRAME_SIZE, 0 );
        if(nByte>0)           
        {
            for(i=0;i<nByte;i++)
            { 
